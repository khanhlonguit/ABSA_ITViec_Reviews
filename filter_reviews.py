...
filter_reviews.py
-----------------
Duyệt qua từng dòng trong file CSV, gọi LLM (Groq, chuẩn OpenAI API)
...
# ---------------------------------------------------------------------------
logging.basicConfig(
    filename="filter_reviews_errors.log",
    filemode="a",
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Lọc review thật / không phải review bằng LLM (Groq)")
    ...
    for i, idx in enumerate(tqdm(rows_to_process, desc="Lọc review", unit="dòng")):
        ...
        df.at[idx, "filter_reason"] = result["reason"]
